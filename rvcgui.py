...
def on_button_click():
    ...
    if model_loaded == True and os.path.isfile(input_audio):
        try:
            ...
            print(e)
            message = "La conversion vocale a échoué", e
    ...
    # Update the output label with the result
       # output_label.configure(text=result + "\n saved at" + output_file)
        ...
        message = "Veuillez sélectionner un modèle et un fichier audio d'entrée"
        run_button.configure(state="enabled")
        result_state.configure(text_color="red")
...
root.title("RVC GUI VF (Traduit par Zxldrok)")
...
gui_dimensions = f"1000x{gui_height}"
    ...
    if pth_files:
        ...
        if npy_files:
            ...
            if len(npy_files_dir) == 1:
                ...
                print(f"Répertoire de fichiers .pth: {pth_file_path}")
                print(f"Répertoire du fichier .index: {index_file}")
                file_index_entry.insert(0, os.path.normpath(index_file))
            else:
                print(f"Ensemble incomplet de fichiers .index trouvés dans {model_dir}")
        else:
            print(f"Aucun fichier .index trouvé dans {model_dir}")
        get_vc(pth_file_path, 0)
        global model_loaded
        model_loaded = True
    else:
        print(f"Aucun fichier .pth éligible trouvé dans {model_dir}")


def index_slider_event(value):
    index_rate_label.configure(
        text='Taux de récupération des fonctionnalités : %s' % round(value, 2))
   # print(value)
...
select_model = ctk.StringVar(value="Sélectionnez un modèle")
...
input_audio_label = ctk.CTkLabel(inputpath_frame, text="Fichier audio d'entrée :")
browse_button = ctk.CTkButton(
    inputpath_frame, text="Parcourir", command=browse_file)
...
file_index_label = ctk.CTkLabel(right_frame, text="Fichier .index (recommandé)")
...
index_rate_label = ctk.CTkLabel(
    right_frame, text="Taux de récupération des fonctionnalités : 0.4" )

# intiilizing run button widget
run_button = ctk.CTkButton(
    left_frame, fg_color="green", hover_color="darkgreen", text="Convertir", command=start_processing)

# intiilizing output label widget
output_label = ctk.CTkLabel(right_frame, text="")

# intiilizing Notes label widget
notes_label = ctk.CTkLabel(left_frame, justify="left", text_color="#8A8A8A", text="Conseils: \n 1. harvest et crepe sont les plus haute qualité, mais aussi des méthodes les plus lentes. \n 2. dio et pm sont les méthodes les plus légères et les plus rapides, mais aussi la moins bonne qualité.")

# intiilizing loading progress bar widget

loading_frame = ctk.CTkFrame(master=root, width=200) 

laoding_label = ctk.CTkLabel(loading_frame, text="Conversion..., Si la fenêtre ne répond pas, veuillez patienter.")
laoding_label.pack(padx=10, pady=10)
loading_progress = ctk.CTkProgressBar(master=loading_frame, width=200)
loading_progress.configure(mode="indéterminé")
...
change_device_label = ctk.CTkLabel( right_frame, text="Mode de traitement")
...
last_output_label = ctk.CTkLabel(output_audio_frame, text="Chemin de sortie : ")
last_output_file = ctk.CTkLabel(output_audio_frame, text="", text_color="green")
open_output_button = ctk.CTkButton(output_audio_frame, text="Ouvrir", command=lambda: play_audio(output_file))

# intiilizing import models button widget
import_moodels_button = ctk.CTkButton(right_frame, fg_color="darkred", hover_color="black", corner_radius=20, text="Importer le modèle depuis .zip", command=browse_zip)
